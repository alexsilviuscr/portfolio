...
export default function Timeline({ showEducation }) {
    const timelineData = showEducation ? timelineEducationData : timelineExperienceData;
    const icon = showEducation ? <MdSchool /> : <MdWork />;
    const timelineClassName = showEducation ? `${styles.timeline} ${styles.education}` : `${styles.timeline} ${styles.experience}`;

  return (
    <VerticalTimeline layout="1-column-left" lineColor={'var(--accent-color-grey)'} className={timelineClassName}>
      {timelineData.map((item, index) => (
        <VerticalTimelineElement
          key={index}
          className={styles.timelineElement}
          contentStyle={{ background: 'var(--secondary-bg-color)', color: 'var(--body-text-color)' }}
          contentArrowStyle={{ borderRight: '7px solid  var(--secondary-bg-color)' }}
          date={item.date}
          iconStyle={{ background: 'var(--secondary-bg-color)', color: 'var(--body-text-color)' }}
